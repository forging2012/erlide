...
public abstract class AbstractConnection extends Thread {
    ...
    protected void doAccept() throws IOException, OtpAuthException {
        try {
            ...
            close();
            final IOException ioe = new IOException("Error accepting connection from "
                    + nn);
            ioe.initCause(e);
            throw ioe;
        }
    ...
    protected void sendChallenge(final int dist, final int aflags, final int challenge)
        ...
        if (traceLevel >= handshakeThreshold) {
            System.out
                    .println("-> " + "HANDSHAKE sendChallenge" + " flags=" + aflags
                            + " dist=" + dist + " challenge=" + challenge + " local="
                            + localNode);
        }
    }
    ...
    protected int recvChallengeReply(final int our_challenge) throws IOException,
        ...
        if (traceLevel >= handshakeThreshold) {
            System.out.println("<- " + "HANDSHAKE recvChallengeReply" + " from="
                    ...
                    + hex(her_digest) + " local=" + localNode);
        }
