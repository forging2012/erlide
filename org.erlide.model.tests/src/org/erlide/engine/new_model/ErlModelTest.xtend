...
class ErlModelTest extends WorkspaceTest {
	...
	def testErlModel() {
		...
		assertThat((erlProject as IHandle).name, is("Test001"));

		val erlProject2 = erlModel.getProject("Test002")
		assertThat((erlProject2 as IHandle).exists, is(false))
		setUpProject("Test002") // a second project with erlang nature
		assertThat((erlProject2 as IHandle).exists, is(true))
		...
		assertThat(erlProject2.getSourceFiles().length, is(0));
		assertThat(erlProject.getSourceFiles().length, is(1));
	}
